                                                                                   ...
                                                                                   const unsigned int size)
{
    if (idx > 0) {
	  ...
	  auto res = m_idxToIter.find(m_first);
	  if (res != m_idxToIter.end())
	  {
             m_selectBuffer.erase(res->second);
	     m_idxToIter.erase(res);
	     m_first++;
	     if (m_debug)
	     {
		if (m_leftLast > m_first) {
		   auto test = m_idxToIter.find(m_first);
		   if (test == m_idxToIter.end()) {
	  ...
	  auto res = m_idxToIter.find(m_rightFirst);
	  if (res != m_idxToIter.end()) {
              m_selectBuffer.erase(res->second);
	      m_idxToIter.erase(res);
	      m_rightFirst++;
	  ...
	  if (m_leftLast < m_last) {
	     ...
	     auto res = m_idxToIter.find(m_rightFirst);
             if (res != m_idxToIter.end()) {
                auto it = m_selectBuffer.insert(res->second, std::make_pair(input[ms_le_idx], ms_le_idx));
		m_idxToIter.insert(std::make_pair(ms_le_idx,it));
		...
		if (m_first > m_leftLast)
	     ...
	     m_idxToIter.insert(std::make_pair(ms_le_idx, m_selectBuffer.begin()));
	     m_leftLast = ms_le_idx;
	  } 
       } 
       if (ms_re_idx < size) {
          m_selectBuffer.push_front(std::make_pair(input[ms_re_idx], ms_re_idx));
	  m_idxToIter.insert(std::make_pair(ms_re_idx, m_selectBuffer.begin()));
	  m_last = ms_re_idx;
       }
    } 
    else {
	   ...
	   m_idxToIter.insert(std::make_pair(i, m_selectBuffer.begin()));
       }
    ...
    if (m_debug) {
	   ...
	   for (auto i = m_first; i <= m_leftLast; i++) {
	       auto res = m_idxToIter.find(i);
	       if (res != m_idxToIter.end()) {
	          std::cout << res->second->first << ", ";
	       }
	       else {
                   std::cout << "Internal Error!" << std::endl;
	       }
...
int main(int argc, char *argv[]) {
   ...
   run_test(1, 2, 1, 0.75, size1, input1, truth1, true);
