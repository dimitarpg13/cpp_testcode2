...
std::deque<Entry>::const_iterator OrderedStatCfarImpl::findSlot(const unsigned int idx) {
	return std::find_if(m_selectBuffer.begin(), m_selectBuffer.end(), 
		       [&idx](const Entry& arg) { return arg.m_idx == idx; });
    ...
    if (idx > 0) {
	  ...
	  if (m_leftLast < m_last) {
	     if (m_rightFirst <= m_last) {
	         auto res = findSlot(m_rightFirst);
                 if (res != m_selectBuffer.end()) {
                    m_selectBuffer.insert(res, {input[ms_le_idx], (unsigned int) ms_le_idx});
		    ...
		    std::cout << "Internal Error!" << std::endl;
	         ...
	         m_selectBuffer.push_front({input[ms_le_idx], (unsigned int) ms_le_idx});
		 m_leftLast = ms_le_idx;
	     }
	  }
       ...
       if (ms_re_idx < size) {
          m_selectBuffer.push_front({input[ms_re_idx], ms_re_idx});
	  m_last = ms_re_idx;
       }
    } 
    else {
       ...
       for (auto i =  m_maskSelectRightStart_offset; i <= end; ++i) {
           m_selectBuffer.push_front({input[i],i});
       }
...
float OrderedStatCfarImpl::reduceSelectBuffer() {
   ...
   if (m_debug)
      std::cout << "the nth elem: " << m_selectBuffer[nth_el].m_val << std::endl;
   return m_selectBuffer[nth_el].m_val;
}
